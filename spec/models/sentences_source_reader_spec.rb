...
describe SentencesSourceReader do
  ...
  it 'should remove and adjust annotations in tokenized text' do
    ...
    indices_csv = <<~EOS
      ...
      77243	326476	老齢 人口[01] は|1 健康管理 に 益々{ますます} 多く の 出費 が 必要 となる だろう
      98170	305532	彼ら(かれら) は|1 大いに{おおいに} 努力 為る(する){した} が 結局 失敗 為る(する){した}
    EOS
    ...
    expect(sentences[0].tokenized).to eq('道 という|と言う 漢字 総画数 何 画 ですか')
    expect(sentences[1].tokenized).to eq('老齢 人口 健康管理 ますます|益々 多く 出費 必要 となる だろう')
    expect(sentences[2].tokenized).to eq('彼ら おおいに|大いに 努力 した|為る 結局 失敗 した|為る')
  end
end
